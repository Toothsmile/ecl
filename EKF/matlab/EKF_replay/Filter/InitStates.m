...
if (param.control.waitForGps == 1)
    ...
    %如果�?��等待gps，先判断下是否有GPS数据
    %imu_data.time_us > gps_data.time_us(gps_data.start_index) 
    %find(arry,1,'first') 找出arry首次出现1个不�?的数
    %找出gps数据出现时�?的imu的time_us
...
%del_vel：变化的速度�?
%accel_dt:只有1维，我猜他是加�?度的总�?，这里拿x,y,z三个方向的加速度分量与�?的相/，有点归�?��的感�?
...
% induce errors)
%利用重力向量对齐倾斜（如果�?度改变将会导致误差）
quat = AlignTilt(quat,initAccel);%给四元数赋初�?
states(1:4) = quat;

% add a roll, pitch, yaw mislignment 增加�?��rpy的初始差�?
...
if (param.control.waitForGps == 1)
    ...
    states(8:9) = gps_data.pos_ned(gps_data.start_index,1:2);%GPS的高程并没有使用，使用的是气压计的高度观测�?
    if(param.fusion.heightFuseMethod==1)
